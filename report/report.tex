...
% Use system fonts for reliable embedding with XeLaTeX
\setmainfont{DejaVu Serif}
\setsansfont{DejaVu Sans}
\setmonofont{DejaVu Sans Mono}

% Configure listings for better code display (monospaced font from fontspec)
\lstset{
    language=Python,
...
\author{Game Development Report}
\date{\today}
...
The game architecture consists of several well-organized components:
...
\begin{itemize}
    ...
    \item \textbf{Rendering System}: Pygame-based graphics rendering for snake, food, and UI elements
    \item \textbf{Score Tracking}: Real-time score tracking and display system during gameplay
\end{itemize}

\subsection{Key Functions}
The main functions in the implementation include:

\begin{itemize}
    \item \texttt{message()}: Displays text messages on the game screen
    \item \texttt{display\_score()}: Shows current score in the top-left corner
    \item \texttt{check\_game\_over()}: Checks for collision conditions
    \item \texttt{gameLoop()}: Main game execution loop with event handling
